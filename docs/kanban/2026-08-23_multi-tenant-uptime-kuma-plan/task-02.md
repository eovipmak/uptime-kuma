...
- Commit or artifact reference: PR from branch `feat/g0-02-adrs` (#3) + CTO correction PR from `docs/g0-02-adr-cto-review` (#6).
- CTO correction (2026-08-24): ADR-0001's database decision reversed from PostgreSQL to **MariaDB** — redbean-node ~0.3.3 supports only MySQL/MariaDB + SQLite (README + dist contain no Postgres dialect), so Postgres would force an unscoped data-layer rewrite; see PR #6 for evidence. Other ADRs confirmed plan-conformant with accuracy fixes.
