...
class TenantUser extends BeanModel {
     ...
     * SQL: SELECT t.* FROM tenant_user tu JOIN tenant t ON t.id = tu.tenant_id WHERE tu.user_id = ?
     * Consumed by G2 for the post-login tenant picker and resolveTenant() middleware.
     * @param {number} userId ID of the user
     * @returns {Promise<object[]>} List of tenant rows the user has membership in
     */
    static async listForUser(userId) {
        return await R.getAll("SELECT t.* FROM tenant_user tu JOIN tenant t ON t.id = tu.tenant_id WHERE tu.user_id = ? ", [
            userId,
        ]);
    }
}
